...
function renderCanvas() {
    ...
    testSteps.forEach((step, index) => {
        ...
        stepEl.addEventListener('drop', (e) => {
            // ================== THE BUG FIX ==================
            // This handler is ONLY for re-ordering existing steps.
            // If dragStartIndex is null, it means we are trying to drop a NEW item
            // from the toolbox. We must NOT stop propagation - let the event bubble
            // up to the canvas's drop handler so the new item can be added.
            if (dragStartIndex === null) {
                return; // Let the event bubble up to the canvas handler
            }
            // ===============================================

            // Now we know we're reordering, so stop the event from reaching the canvas
            e.stopPropagation();
            e.preventDefault();
