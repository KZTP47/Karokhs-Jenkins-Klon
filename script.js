...
try:
    ...
    result = -1
        ...
        function renderTestSuites(suites) {
            ...
            if (suites.length === 0) {
                container.innerHTML = '<p class="aero-text-muted text-center py-8">No test suites configured. Click "Add New Test Suite" to get started.</p>';
                return;
            }
        ...
        function openAddSuiteModal() {
            ...
            document.getElementById('website-integration-options')?.classList.add('hidden');
            const uploadedPreview = document.getElementById('uploaded-files-preview');
            if (uploadedPreview) uploadedPreview.innerHTML = '';
        ...
        function editSuite(suiteId) {
            ...
            if (suite.language === 'website') {
                toggleWebsiteIntegrationOptions(); // Show website options
                
                const method = suite.website_method || 'url';
                document.querySelector(`input[name="website-method"][value="${method}"]`).checked = true;
                toggleWebsiteMethodInputs();
                
                if (method === 'url') {
                    document.getElementById('website-url').value = suite.website_url || '';
        ...
        async function handleSave(event) {
            ...
            if (suite.language === 'website') {
                ...
                if (method === 'url') {
                    suite.website_url = document.getElementById('website-url').value;
                } else if (method === 'upload') {
...
`;
        ...
        function toggleWebsiteIntegrationOptions() {
            ...
            const websiteOptions = document.getElementById('website-integration-options');
