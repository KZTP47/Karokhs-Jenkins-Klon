...
// Unit Test Object Structure
/*
{
    id: "unittest-timestamp-random",
    name: "Test Name",
    description: "What this test validates",
    suite_id: "linked-suite-id",  // Links to a test suite
    test_framework: "pytest|unittest|jest|junit|nunit|custom",
    test_code: "unit test code",
    setup_code: "setup/fixtures code",
    teardown_code: "cleanup code",
    expected_result: "expected output/behavior",
    tags: ["unit", "integration", "e2e"],
    status: "PASS|FAIL|SKIPPED|NOT_RUN",
    last_run_time: "ISO timestamp",
    last_run_output: "test output",
...
function initializeUnitTestingSystem() {
    try {
        ...
        showMessage('Failed to initialize Unit Testing System', 'error');
    }
}
...
function saveUnitTestFromForm() {
    try {
        ...
        console.log('Saving unit test... isEdit:', isEdit);
    ...
    try {
        ...
        if (titleEl) {
            if (currentUnitTestFilter.suiteId === null) {
                ...
                const suite = testSuites.find(s => s.id === currentUnitTestFilter.suiteId);
            ...
            <div class="mt-3 p-3 aero-glass-panel rounded text-sm">
                ...
                ${test.last_run_error ? `
                    <div class="mt-2 p-2 bg-red-50 border border-red-200 rounded">
                        ...
                        <pre class="text-xs text-red-600 whitespace-pre-wrap">${escapeHtml(test.last_run_error)}</pre>
                    </div>
                ` : ''}
                ${test.last_run_output ? `
                    <div class="mt-2">
                        <button onclick="toggleUnitTestOutput('${test.id}')" 
                            class="aero-button-info py-1 px-3 rounded text-xs">
                            View Output
                        </button>
                        <div id="output-${test.id}" class="hidden mt-2 p-2 bg-blue-50 border border-blue-200 rounded">
                            <pre class="text-xs whitespace-pre-wrap">${escapeHtml(test.last_run_output)}</pre>
                        </div>
                    </div>
                ` : ''}
            </div>
...
function filterUnitTests() {
    currentUnitTestFilter.searchText = document.getElementById('unit-test-search').value.toLowerCase();
    currentUnitTestFilter.statusFilter = document.getElementById('unit-test-status-filter').value;
    currentUnitTestFilter.frameworkFilter = document.getElementById('unit-test-framework-filter').value;
    renderUnitTestsList();
}
...
`;
        ...
        // Simple assertion counting (this is a basic implementation)
        const assertCount = (unitTest.test_code.match(/assert /g) || []).length;
        
        return {
            status: 'PASS',
            output: String(output || 'Test completed successfully'),
            assertions_passed: assertCount,
            assertions_failed: 0
        };
        
    } catch (error) {
        // Count assertions that may have passed before failure
        const totalAsserts = (unitTest.test_code.match(/assert /g) || []).length;
        ...
        return {
            ...
            output: '',
            assertions_passed: 0,
            assertions_failed: totalAsserts
        };
    }
}
    ...
    try {
        ...
        const expect = (actual) => ({
            toBe: (expected) => {
                if (actual === expected) {
                    passed++;
                    outputs.push(`✓ Assertion passed: ${actual} === ${expected}`);
                    return true;
                } else {
                    failed++;
                    outputs.push(`✗ Assertion failed: Expected ${expected}, got ${actual}`);
            ...
            toEqual: (expected) => {
                if (JSON.stringify(actual) === JSON.stringify(expected)) {
                    passed++;
                    outputs.push(`✓ Assertion passed: Objects equal`);
                    return true;
                } else {
                    failed++;
                    outputs.push(`✗ Assertion failed: Objects not equal`);
            ...
            toBeTruthy: () => {
                if (actual) {
                    passed++;
                    outputs.push(`✓ Assertion passed: Value is truthy`);
                    return true;
                } else {
                    failed++;
                    outputs.push(`✗ Assertion failed: Value is not truthy`);
            ...
            toBeFalsy: () => {
                if (!actual) {
                    passed++;
                    outputs.push(`✓ Assertion passed: Value is falsy`);
                    return true;
                } else {
                    failed++;
                    outputs.push(`✗ Assertion failed: Value is not falsy`);
            ...
            toContain: (expected) => {
                if (Array.isArray(actual) && actual.includes(expected)) {
                    passed++;
                    outputs.push(`✓ Assertion passed: Array contains ${expected}`);
                    return true;
                } else {
                    failed++;
                    outputs.push(`✗ Assertion failed: Array does not contain ${expected}`);
        ...
        const assert = (condition, message = 'Assertion failed') => {
            if (condition) {
                passed++;
                outputs.push(`✓ Assertion passed: ${message}`);
            } else {
                failed++;
                outputs.push(`✗ Assertion failed: ${message}`);
    ...
    try {
        ...
        for (const test of filteredTests) {
            try {
                ...
                console.log(`Progress: ${completed}/${filteredTests.length}`);
